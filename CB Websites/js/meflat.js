$(document).ready(function() {
	...
	if ($(window).scrollTop()===0){
		$('#telephone-content').removeClass('scrolled');
		$('#main-nav').removeClass('scrolled');
	}
	else{
		$('#telephone-content').addClass('scrolled');
	...
	$('.project-item').click(function(e){
		...
		   if (slides[i].indexOf(".jpg") != -1)
		   {
		       slidesHtml = slidesHtml + '<li><img src='+slides[i]+' alt=""></li>';
		   }else if (slides[i].indexOf(".swf") != -1)
		   {
		       slidesHtml = slidesHtml + '<li><object type="application/x-shockwave-flash" data='+slides[i]+' width="300" height="250"><param name="movie" value='+slides[i]+' /><param name="quality" value="high"/></object></li>';
		   }
		}
		
		slidesHtml = slidesHtml + '</ul>';
		// BEGIN: Additional Codes by Den
		var videoBoolean = (elem.context.className === 'project-item html5 masonry-brick');
		if(videoBoolean) {
			$('div.image-wrapper').css( "display", "none");
			$('div#video-container').css( "display", "block");
		}
		// END:
	...
	$('#project-modal').on('hidden.bs.modal', function () {
		...
		var iframe 	= $('#vimeo-player-project')[0];
		var player 	= $f(iframe);
		$('div.image-wrapper').css( "display", "block");
		$('div#video-container').css( "display", "none");
